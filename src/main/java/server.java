...
public class server {
  ...
  private void run(String portNumber) {
    ...
    server.createContext("/person", new PersonHandler()); //returns all peoples as an array

    server.createContext("/event", new EventHandler());

    server.createContext("/load", new LoadHandler()); //what does it want???
    ...
    // (e.g., web site files)
    //todo do this one
    server.createContext("/", new FileHandler());   //what to do with this???
